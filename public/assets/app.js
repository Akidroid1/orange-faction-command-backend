const CONFIG={factionId:53295,discordInvite:'https://discord.gg/duHJkWRRT',tornStats:'https://www.tornstats.com/factions/53295',refreshMs:300000};
const $=s=>document.querySelector(s), fmt=n=>n==null||n===''||Number.isNaN(Number(n))?'—':Number(n).toLocaleString(), esc=s=>String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[c]));
let DATA={};
...
function fmtTime(v){
  if(v==null||v==='')return '—';
  if(typeof v==='number'){
    try{return new Date(v*1000).toLocaleString()}catch{}
  }
  return String(v)
}
...
function stateCount(ms){
  return ms.filter(m=>{
    const s=String(get(m,'status.state','status','state')||'').toLowerCase();
...
function render(d){
  ...
  $('#heroSub').textContent=`Faction #${CONFIG.factionId} • Command intelligence synchronized from Torn`;

  const respect=get(b,'respect','respect_value');

  $('#respect').textContent=fmt(respect);
  $('#membersCount').textContent=fmt(get(b,'members.member_count','member_count')??ms.length);
  ...
  $('#chainBig').textContent=fmt(get(ch,'current','current_chain','chain'));
  ...
  $('#factionInfo').innerHTML=info.map(x=>
    `<div><span>${esc(x[0])}</span><b>${esc(x[1])}</b></div>`
  ).join('');

  const chainNow=Number(get(ch,'current','current_chain','chain'))||0;
  const chainMax=Number(get(ch,'max','max_chain'))||0;

  $('#chainBar').style.width=
    (chainMax
      ? Math.min(100,chainNow/chainMax*100)
      : Math.min(100,chainNow/100)
    )+'%';

  $('#chainTimeout').textContent=
    `Timeout ${esc(get(ch,'timeout','chain.timeout')||'—')}`;

  $('#chainCooldown').textContent=
    `Cooldown ${esc(get(ch,'cooldown','chain.cooldown')||'—')}`;

  $('#chainDetails').innerHTML=[
    ['Current chain',fmt(chainNow)],
    ['Maximum chain',fmt(chainMax)],
    ['Timeout',get(ch,'timeout','chain.timeout')||'—'],
    ['Cooldown',get(ch,'cooldown','chain.cooldown')||'—'],
  ...
  const readiness=Math.min(
    ...
    Math.round(
      (ms.length
        ? Math.min(1,stateCount(ms)/Math.max(1,ms.length))*.4
        : .2
      )*100+
  ...
  $('#readinessTitle').textContent=
    readiness>=75?'Combat ready':
    readiness>=45?'Operational':
    'Standby';
  ...
  $('#coverageInfo').innerHTML=keys.map(k=>
    `<div class="coverage-row">
      <span>${esc(k)}</span>
      <i class="${d[k]?'ok':'no'}">${d[k]?'●':'○'}</i>
    </div>`
  ).join('');
...
function renderMembers(ms){
  ...
  $('#membersTable').innerHTML=
    ms.filter(m=>JSON.stringify(m).toLowerCase().includes(q))
    .map(m=>
      `<tr>
        <td>
          <a href="https://www.torn.com/profiles.php?XID=${encodeURIComponent(m.id)}" target="_blank" rel="noopener">
            ${esc(m.name||m.username||m.id)}
          </a>
        </td>
        <td>${esc(m.level??'—')}</td>
        <td>${esc(get(m,'position.name','position','role')||'—')}</td>
        <td>${esc(get(m,'last_action.relative','last_action.timestamp','last_action')||'—')}</td>
        <td><span class="pill">${esc(get(m,'status.state','status','state')||'—')}</span></td>
      </tr>`
    ).join('')
    ||
    '<tr><td colspan="5">No member data returned.</td></tr>';
}
...
function renderCrimes(cs){
  ...
  $('#crimesTable').innerHTML=
    cs.slice(0,150).map(c=>
      `<tr>
        <td>${esc(c.name||c.crime_name||c.id||'OC')}</td>
        <td><span class="pill">${esc(c.status||c.state||'—')}</span></td>
        <td>${esc(fmtTime(c.created_at||c.created))}</td>
        <td>${fmt(arr(c.participants||c.slots).length)}</td>
        <td>${esc(c.difficulty||c.success||'—')}</td>
      </tr>`
    ).join('')
    ||
...
function renderArmory(items){
  ...
  $('#armoryTable').innerHTML=
    ...
    .map(i=>
      `<tr>
        <td>${esc(i.name||i.item_name||i.id||'Item')}</td>
        <td>${esc(i.type||i.category||'—')}</td>
        <td>${fmt(i.quantity??i.qty??i.amount)}</td>
        <td>${esc(i.id??'—')}</td>
      </tr>`
    ).join('')
    ||
  ...
  $('#upgradesInfo').innerHTML=
    us.map(u=>
      `<div class="upgrade-card">
        <b>${esc(u.name||u.upgrade||u.id)}</b>
        <span>Level ${esc(u.level??u.current_level??'—')}</span>
      </div>`
    ).join('')
    ||
...
async function load(){
  ...
  try{
    ...
    if(j._meta?.partial_failures?.length){
      e.textContent=
        `Some Torn feeds were unavailable: ${j._meta.partial_failures.join(' • ')}`;

      e.classList.remove('hidden');
    }

  }catch(x){
    e.textContent=`Could not load Torn data: ${x.message}`;
    e.classList.remove('hidden');
...
document.querySelectorAll('.nav').forEach(b=>
  b.addEventListener('click',()=>{
    document.querySelectorAll('.nav,.tab-panel')
